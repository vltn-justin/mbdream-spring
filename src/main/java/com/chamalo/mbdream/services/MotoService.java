...
public class MotoService {
    ...
    @Autowired
    public MotoService(final MotoRepository motoRepository, @Lazy final MarqueService marqueService,
    ...
    public MotoModel findMotoBySlug(final String slug) {
        return this.motoRepository.findMotoBySlug(slug)
                .orElseThrow(() -> new MBDreamException("Moto introuvable avec le slug " + slug));
    }
     ...
     * @param motoDTO MotoRequest with all data
     */
    public void updateMoto(final MotoDTO motoDTO) {
        ...
        // Si la marque change
        if (updatedMoto.getMarque() == null || !updatedMoto.getMarque().getSlugMarque()
                .equals(motoDTO.getSlugMarque())) {
            updatedMoto.setMarque(this.marqueService.findMarqueBySlug(motoDTO.getSlugMarque()));
        }

        // Si la catégorie change
        if (updatedMoto.getCategorie() == null || !updatedMoto.getCategorie().getSlugCategorie()
                .equals(motoDTO.getSlugCategorie())) {
            updatedMoto.setCategorie(this.categorieService.findCategorieBySlug(motoDTO.getSlugCategorie()));
        }

        this.motoRepository.save(updatedMoto);
    }
